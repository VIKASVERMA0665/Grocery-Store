...
{(["deals","products","search","grocery","fresh food","dairy & bakery","beverages","household","personal care"].includes(page))&&<><div className="head"><div><h1>{page==="search"&&q?("Search: "+q):page==="deals"?"Deals":"Products"}</h1><p>{filtered.length} products</p></div></div><div className="grid">{filtered.map((p,i)=><Card key={i} p={p} add={add} i={i}/>)}</div></>}
...
{page==="orders"&&<><h1>My orders</h1><div className="order"><b>Order #AFB1001</b><span>Out for delivery</span><small>Order tracking will connect to Firestore.</small></div></>}
