...
function App(){
 ...
 {page==="cart"&&<><h1>Your cart</h1>{!cart.length?<div className="empty">🛒<h2>Your cart is empty</h2><button className="primary" onClick={()=>setPage("categories")}>Start shopping</button></div>:<div className="cart"><div>{cart.map((p,i)=><div className="cartitem" key={i}><span>{p.emoji||"🛒"}</span><div><b>{p.name}</b><small>{p.unit}</small></div><strong>₹{p.price}</strong><button onClick={()=>setCart(c=>c.filter((_,j)=>j!==i))}>Remove</button></div>)}</div><aside><h2>Order summary</h2><p>Subtotal <b>₹{cart.reduce((s,p)=>s+p.price,0)}</b></p><button className="primary" onClick={()=>setPage("checkout")}>Checkout</button></aside></div>}</>}
