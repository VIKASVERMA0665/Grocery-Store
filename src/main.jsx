...
function App(){
 ...
 {page==="cart"&&<><h1>Your cart</h1>{!cart.length?<div className="empty">🛒<h2>Your cart is empty</h2><button className="primary" onClick={()=>setPage("categories")}>Start shopping</button></div>:<div className="cart"><div>{cart.map((p,i)=><div className="cartitem" key={p.id||i}><span>{p.emoji||"🛒"}</span><div><b>{p.name}</b><small>{p.unit}</small></div><strong>₹{p.price*(p.qty||1)}</strong><div className="qty-controls"><button type="button" onClick={()=>setCart(c=>c.flatMap((x,j)=>j!==i?x:(x.qty||1)>1?[{...x,qty:x.qty-1}]:[]))}>−</button><b>{p.qty||1}</b><button type="button" onClick={()=>setCart(c=>c.map((x,j)=>j===i?{...x,qty:(x.qty||1)+1}:x))}>＋</button></div><button onClick={()=>setCart(c=>c.filter((_,j)=>j!==i))}>Remove</button></div>)}</div><aside><h2>Order summary</h2><p>Subtotal <b>₹{cart.reduce((s,p)=>s+p.price*(p.qty||1),0)}</b></p><button className="primary" onClick={()=>setPage("checkout")}>Checkout</button></aside></div>}</>}
